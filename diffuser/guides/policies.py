...
class GuidedPolicy:
    ...
    def __call__(self, conditions, batch_size=1, verbose=True):
        ...
        ## run reverse diffusion process
        sample = self.diffusion_model(conditions, guide=self.guide, verbose=verbose, **self.sample_kwargs)
        sample = utils.to_np(sample)

        ## extract action [ batch_size x horizon x transition_dim ]
        actions = utils.to_np(sample.trajectories[:, :, :self.action_dim])
        actions = self.normalizer.unnormalize(actions, 'actions')

        ## extract first action
        action = actions[0, 0]

        normed_observations = utils.to_np(sample.trajectories[:, :, self.action_dim:])
        observations = self.normalizer.unnormalize(normed_observations, 'observations')

        trajectories = GuidedTrajectories(actions, observations, utils.to_np(sample.values))
        return action, trajectories
